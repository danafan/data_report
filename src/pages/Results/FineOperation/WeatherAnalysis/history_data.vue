	...
	<div>
		<el-form :inline="true" size="small" class="demo-form-inline">
			<el-form-item label="日期选择：">
				<el-date-picker v-model="date" align="right" type="daterange" unlink-panels value-format="yyyy-MM-dd" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :append-to-body="false" :picker-options="pickerOptions" @change="dateChange">
				</el-date-picker>
			</el-form-item>
		</el-form>
			...
			getData(){
				resource.weatherData(this.req).then(res => {
					if(res.data.code == 1){
						...
						var temperature_his = document.getElementById('temperature_his');
						if(this.temperature_hisChart){
							this.temperature_hisChart.dispose();
						}
						this.temperature_hisChart = echarts.init(temperature_his);
						this.temperature_hisChart.setOption(this.temperatureOptions(zgw,zdw,avg_wd));

						//未来每日风量
						var fl = data.day_weather.fl;			//风量
						var wind_speed_his = document.getElementById('wind_speed_his');
						if(this.wind_speed_hisChart){
							this.wind_speed_hisChart.dispose();
						}
						this.wind_speed_hisChart = echarts.init(wind_speed_his);
						this.wind_speed_hisChart.setOption(this.flOptions(fl));
						...
						var bar_content_his = document.getElementById('bar_content_his');
						if(this.bar_content_hisChart){
							this.bar_content_hisChart.dispose();
						}
						this.bar_content_hisChart = echarts.init(bar_content_his);
						this.bar_content_hisChart.setOption(this.barOptions(left_legend,left_series));

						//穿衣气象指数统计环形图
						var pie_content_his = document.getElementById('pie_content_his');
						if(this.pie_content_hisChart){
						...
						window.addEventListener('resize',() => {
							...
							this.bar_content_hisChart.resize();
							this.pie_content_hisChart.resize();
						})
					}else{
