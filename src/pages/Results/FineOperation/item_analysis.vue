<template>
	<div>
			...
			<div class="form_box">
				<el-form :inline="true" size="small" class="demo-form-inline">
					...
					<el-form-item label="店铺：">
						<el-select v-model="select_shop_list" clearable :popper-append-to-body="false" multiple filterable remote reserve-keyword placeholder="请输入店铺" :remote-method="getStore" collapse-tags>
				...
				<div class="bottom_toast">
					<div class="item_row">
						<div class="dian"></div>
						<div class="toast_title">每日数据更新时间：13:00</div>
					</div>
					<div class="item_row">
						<div class="dian"></div>
		...
		<el-table :data="table_list.data" size="small" style="width: 100%" :header-cell-style="{'background':'#8D5714','color':'#ffffff'}" height='600' :cell-style="columnStyle">
			<el-table-column :label="item.row_name" :prop="item.row_field_name" :width="item.row_field_name == 'yxgxmyl'?160:120" v-for="item in title_list" :sortable="item.is_sort?'custom':false" show-overflow-tooltip :fixed="zbhzFixed(item.row_field_name)">
				<template slot-scope="scope">
					<!-- 占比 -->
					<div class="background_box" :class="{'is_negative':scope.row[item.row_field_name] < 0}" :style="{width:`${item.max_value == 0?0:((item.row_field_name == 'yxgxmyl'?160:120)/item.max_value)*Math.abs(scope.row[item.row_field_name])}px`}" v-if="item.type == 1 && scope.row.is_total != 1">{{scope.row[item.row_field_name]}}{{item.unit}}</div>
					<!-- 图片 -->
	...
	export default{
		data(){
			return{
				...
				visitorsAnalysisChart:null,					//访客分析折线图
		...
		methods:{
			...
			dpAnalysis(){
				...
				resource.dpAnalysis(req).then(res => {
					if(res.data.code == 1){
			...
			//每日分析下面的折线图
			dayDpAnalysis(){
				resource.dayDpAnalysis().then(res => {
					if(res.data.code == 1){
