<template>
	<div>
		...
		<el-form size="small" style="max-height: 700px;overflow-y: scroll">
			<el-form-item label="店铺：" required>
				<el-select v-model="store_id" :popper-append-to-body="false" placeholder="全部">
			...
			<el-form-item label="一级类目：" required>
				<el-select v-model="level_1_cate" :popper-append-to-body="false" placeholder="全部">
					<el-option v-for="item in store_list" :key="item.id" :label="item.name" :value="item.id">
					</el-option>
				</el-select>
			</el-form-item>
			...
			<el-input style="width:200px;margin-bottom: 15px" type="textarea"
			placeholder="请输入文字描述"
			:rows="3"
			v-model="bmgy_remark"
			maxlength="100"
			...
			<el-form-item label="款式需要数量：" required>
				<el-input style="width:200px" clearable v-model="ksxysl" placeholder="款式需要数量"></el-input>
			</el-form-item>
