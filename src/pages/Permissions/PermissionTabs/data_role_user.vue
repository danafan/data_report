		...
		<el-table size="small" :data="dataObj.data" tooltip-effect="dark" style="width: 100%" :header-cell-style="{'background':'#f4f4f4'}" v-loading="loading">
			...
			<el-table-column label="操作" align="center">
				<template slot-scope="scope">
					...
					<el-button type="text" size="small" @click="getDetail(scope.row.user_id)" v-if="dataObj.button_list.detail == '1' && scope.row.is_self == '0'">查看</el-button>
					<el-button type="text" size="small" @click="deleteFun(scope.row.user_id)" v-if="dataObj.button_list.del == '1' && scope.row.is_self == '0'">删除</el-button>
				</template>
			</el-table-column>
		</el-table>
		<div class="page">
...
<el-dialog title="基本信息" width="30%" :visible.sync="show_detail">
	...
	<div class="detail_item">
		...
		<div class="item_val">{{user_detail.data_role_time}}</div>
	</div>
	<div slot="footer" class="dialog-footer">
	...
	export default{
		...
		methods:{
			...
			settingFun(user_id){
				...
				resource.setDataRoleGet({user_id:this.user_id}).then(res => {
					if(res.data.code == 1){
						this.show_setting = true;
						this.ding_user_name = res.data.data.ding_user_name;
						this.set_time_id = res.data.data.data_role_time == '-1'?'-1':'1';
						this.over_time = res.data.data.data_role_time == '-1'?'':res.data.data.data_role_time;
					}else{
						this.$message.warning(res.data.msg);
					}
				})
			...
			submitSetting(){
				...
				resource.setDataRolePost(settingReq).then(res => {
					if(res.data.code == 1){
			...
			//查看
			getDetail(user_id){
				resource.dataRoleUseInfo({user_id:user_id}).then(res => {
					if(res.data.code == 1){
			...
			deleteFun(user_id){
				this.$confirm('确认删除该用户?', '提示', {
					...
					let req = {
						user_id:user_id,
						role_id:this.req.rold_id
					}
					resource.dataRoleMoveUser(req).then(res => {
						if(res.data.code == 1){
