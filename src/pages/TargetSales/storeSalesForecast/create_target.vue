	...
	export default{
		...
		methods:{
						...
						for(var k in this.lastYearData){
							for(let i = 0;i < this.table_data.length;i ++){
								if(this.table_data[i].key == k){
									...
									if(this.table_data[i].key == 'kdj' || this.table_data[i].key == 'tkl' || this.table_data[i].key == 'xssr' || this.table_data[i].key == 'mll' || this.table_data[i].key == 'yxfyl'){
			...
			changeInput(v,item){
				if(item.key == 'kdj'){
					// 预估发货单数（本月销售收入/客单价）
					this.table_data[0].new_value = this.table_data[2].new_value == 0 || this.table_data[4].new_value == 0?0:this.table_data[2].new_value === '' || !this.table_data[2].new_value || this.table_data[4].new_value === '' || !this.table_data[4].new_value?'':(this.table_data[4].new_value/this.table_data[2].new_value).toFixed(2);
