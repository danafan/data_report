<template>
	<div>
		<condition page_type="3" @callBack="searchFun"/>
		...
		methods:{
			...
			exportFile(){
				...
				MessageBox.confirm('确认导出?', '提示', {
					...
					resource.stopExport(this.req).then(res => {
						exportPost("\ufeff" + res.data,'停止进货管理');
					})
				}).catch(() => {
					Message({
						type: 'info',
			...
			getList(){		
				...
				this.req.sort = this.sort;
				this.req.sort_type = this.sort_type;
				resource.stopList(this.req).then(res => {
					if(res.data.code == 1){
			...
			//设置自定义列
			setColumns(){
				resource.setColumns({menu_id:'20',row_ids:this.row_ids.join(',')}).then(res => {
					if(res.data.code == 1){
						this.$message.success(res.data.msg);
						this.getList();
					}else{
